...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
    ({ className = '', label, error, leftIcon, rightIcon, ...props }, ref) => {
        return (
            <div className={`w-full ${className.includes('liquid-glass') ? '' : className}`}>
                {label && (
                    <label className={`block text-[11px] font-black mb-2.5 uppercase tracking-[0.2em] transition-colors ${error ? 'text-accent-danger' : 'text-ui-text'}`}>
                ...
                <div className="relative group">
                    ...
                    <input
                        ...
                        className={`
                            ...
                            placeholder:text-ui-text-muted/60 placeholder:font-bold placeholder:uppercase placeholder:text-[10px] placeholder:tracking-[0.15em]
                            focus:bg-white dark:focus:bg-white/10 focus:ring-4 focus:ring-accent-primary/10 focus:border-accent-primary
                            disabled:cursor-not-allowed disabled:opacity-40
                            ${className.includes('liquid-glass') ? 'liquid-glass !border-0' : ''}
                            ${leftIcon ? 'pl-14' : 'px-5'}
