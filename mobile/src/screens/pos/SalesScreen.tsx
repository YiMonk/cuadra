...
import { Chip, Portal, Modal, List, Divider, RadioButton } from 'react-native-paper';
...
export default function SalesScreen({ navigation }: Props) {
    ...
    const handleAddToCart = (item: Product, variant?: ProductVariant) => {
        if (!variant && item.variants && item.variants.length > 0) {
    ...
    const handleRemoveOne = (productId: string) => {
        // This is tricky with variants. For now, this simply decreases quantity of the first matching item found
        // or we could disable it for products with variants.
        // Better UX: If multiple variants, maybe alert "Gestionar desde el carrito".
        // For simple products, it works.
        const item = items.find(i => i.id === productId);
        if (item) {
            // If it has variants, we don't know which one to remove.
            // Ideally we should open the cart summary.
            if (item.variantId) {
                // For now doing nothing or we could remove the specific one if we knew it.
                // Let's just update the first one found.
                updateQuantity(productId, item.quantity - 1, item.variantId);
            } else {
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
            ...
            <Portal>
                <Modal
                    ...
                    onDismiss={() => setVariantModalVisible(false)}
                    contentContainerStyle={[styles.modalContent, { backgroundColor: theme.colors.surface }]}
                >
                    <View style={styles.modalHeader}>
                        <Text variant="titleLarge">Seleccionar Variante</Text>
                        <IconButton icon="close" onPress={() => setVariantModalVisible(false)} />
                    </View>

                    {selectedProductForVariant && (
                        <View>
                            <Text variant="titleMedium" style={{ marginBottom: 10 }}>{selectedProductForVariant.name}</Text>
                            <ScrollView style={{ maxHeight: 300 }}>
                                {selectedProductForVariant.variants?.map(variant => (
                                    <List.Item
                                        key={variant.id}
                                        title={variant.name}
                                        description={`Stock: ${variant.stock}`}
                                        onPress={() => handleAddToCart(selectedProductForVariant, variant)}
                                        right={props => <IconButton {...props} icon="plus-circle" iconColor={theme.colors.primary} />}
                                        style={{ backgroundColor: theme.colors.surfaceVariant, marginBottom: 8, borderRadius: 8 }}
                                    />
                                ))}
                            </ScrollView>
                        </View>
